  ...
  <v-dialog transition="dialog-top-transition" max-width="300">
    <template v-slot:activator="{ on: card, attrs }">
      <v-tooltip top>
        ...
        <span class="poppins">Delete Sale</span>
      </v-tooltip>
    </template>
    <template v-slot:default="dialog">
      <v-card>
        ...
        <v-card-actions class="justify-end">
          <v-btn text @click="deleteSale(sale)" color="primary">Yes</v-btn>
...
export default {
  ...
  methods: {
    deleteSale(sale) {
      this.$store.dispatch("modules/sales/deleteSales", sale);
